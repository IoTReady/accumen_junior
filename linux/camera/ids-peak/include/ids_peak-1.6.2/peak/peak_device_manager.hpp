...
T& DeviceManager::LifetimeTracker<T>::Instance()
{
    ...
    {
        throw core::NotInitializedException("Library is not initialized. Call peak::Library::Initalize() first!");
    }
    return *m_instance;
}
...
{
    return DeviceManager::LifetimeTracker<>::Instance();
}
    ...
    const UpdateErrorCallback& errorCallback /*= UpdateErrorCallback()*/)
{
    ...
    // We apply the update changes to our internal lists
    {
        ...
        std::lock_guard<std::recursive_mutex> lock2(m_interfacesMutex);
        std::lock_guard<std::recursive_mutex> lock3(m_devicesMutex);
...
inline void DeviceManager::Reset(ResetPolicy resetPolicy)
{
...

    {
        ...
        {
            auto origDevices = m_devices;
            m_devices.clear();
            for (const auto& lostDevice : origDevices)
            {
        ...
        {
            auto origInterfaces = m_interfaces;
            m_interfaces.clear();
            for (const auto& lostInterface : origInterfaces)
            {
