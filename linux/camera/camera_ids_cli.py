...
def save_image(image_byte, filename,window):
    ...
    # sleep(5)
    display_image(window,f_name)
    

def display_image(window,f_name):
    # Convert the image to a PhotoImage object
    # image = Image.open(f_name)
    photo_image = tk.PhotoImage(file=f_name)
    image_label = tk.Label(window, image=photo_image)
    image_label.pack()



def continous_shot(dev,nmap,window):    
    global filename
    img = capture_optimised(dev,nmap)
    save_image(img,filename,window)
    filename += 1
...
def gui_land(dev,nmap):
     # Initialize GUI window 
    window = tk.Tk()
    window.title("IDS Camera App")
    ...
    # Capture button
    btn_capture = tk.Button(window, text="Capture", command=lambda: continous_shot(dev,nmap,window)) 
    # btn_capture = tk.Button(window, text="Capture", command=lambda: dummy(window)) 
    btn_capture.pack(side=tk.BOTTOM, pady=10)

    btn_exit = tk.Button(window, text="Exit", command=lambda: exit(window)) 
    btn_exit.pack(side=tk.BOTTOM, pady=10)

    image_path = "pd0.png"
    pil_image = Image.open(image_path)
    small_image = pil_image.resize((400, 400), Image.ANTIALIAS)
    photo_image = tk.PhotoImage(small_image)
